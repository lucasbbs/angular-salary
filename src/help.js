...
function calculateDiscountINSS(baseINSS) {
  ...
  for (var i = 0; i < INSS_TABLE.length; i++) {
    ...
    if (baseINSS > currentItem.maxValue) {
      // prettier-ignore
      discountValue = (currentItem.difference * currentItem.discountPercentage);

      discountINSS += discountValue;
    } else {
      // prettier-ignore
      discountValue = ((Number(baseINSS) - currentItem.minValue) * currentItem.discountPercentage);
...
function calculateSalaryFrom(fullSalary, dependentes) {
  ...
  const dependentesDiscount =
    baseINSS > 189.59 * Number(dependentes) ? 189.59 * Number(dependentes) : 0;
  const percentageDependentes = calculatePercentage(
    dependentesDiscount,
    baseINSS
  );
  const baseIRPF = baseINSS - discountINSS - dependentesDiscount;
